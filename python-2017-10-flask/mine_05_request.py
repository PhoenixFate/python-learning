...
def view_request():
    ...
    # 通过request.form可以直接提取请求体中表单格式的数据，是一个字段对象

    # request.form从post发送的表单中获取数据
    # 如果你前端是表单格式发送的，则需要用request.form.get来获取数据
    name = request.form.get("name")
    age = request.form.get("age")
    # 如果前端是json格式发送的，则需要用request.data来获取数据
    print("request.data: " + str(request.data))
    # request.args 记录请求中i的查询参数：/index?city=beijing ?之后的称为查询字符串(QueryString)
    print("request.args: " + str(request.args))
    city = request.args.get("city")

    print(type(request.headers))
    print("request.headers:"+str(request.headers))
    return "从request中获取参数; name=%s, age=%s, city=%s" % (name, age, city)
